...
automation_platform=$(prompt_for_input "Enter automation platform to install? (ascender/awx): " "ascender")
# Bail if platform is not supported
if [ "$automation_platform" != "ascender" ] && [ "$automation_platform" != "awx" ]; then
    echo "Unsupported automation platform: $automation_platform"
    exit 1
fi
...
awx_operator_resource="github.com/ansible/awx-operator/config/default?ref=$operator_version"
ascender_operator_resource="github.com/ciq/ascender-operator/config/default?ref=$operator_version"
echo ""
echo ""
...
cp "./templates/stage_two_automation-platform.yaml" "./stage_two/automation-platform.yaml"
cp "./templates/stage_two_automation-platform-ingress.yaml" "./stage_two/automation-platform-ingress.yaml"
...
sed -i "s/<ADMIN_USER>/$username/g" ./stage_two/automation-platform.yaml
sed -i "s/<AWX_FQDN>/$awx_fqdn/g" ./stage_two/automation-platform-ingress.yaml
sed -i "s/<NAMESPACE>/$namespace/g" ./stage_two/automation-platform-ingress.yaml
if [ "$automation_platform" == "ascender" ]; then
    sed -i "s/<OPERATOR_RESOURCE>/$ascender_operator_resource/g" ./stage_one/kustomization.yaml
    sed -i "s/<OPERATOR_RESOURCE>/$ascender_operator_resource/g" ./stage_two/kustomization.yaml
elif [ "$automation_platform" == "awx" ]; then
    sed -i "s/<OPERATOR_RESOURCE>/$awx_operator_resource/g" ./stage_one/kustomization.yaml
    sed -i "s/<OPERATOR_RESOURCE>/$awx_operator_resource/g" ./stage_two/kustomization.yaml
fi
